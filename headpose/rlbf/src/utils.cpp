...
void LoadImages(std::vector<cv::Mat_<uchar> >& images,
	...
	while (fin >> name){
        ...
        cv::Mat_<uchar> image = cv::imread(("/home/jxgu/dataset/ibug/helen/trainset/" + name).c_str(), 0);
        cv::Mat_<double> ground_truth_shape = LoadGroundTruthShape(("/home/jxgu/dataset/ibug/helen/trainset/" + pts).c_str());
